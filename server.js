...
// Verifica se o nickname já está em uso (case-insensitive)
function nicknameEmUso(username) {
  const alvo = username.toLowerCase();
  return [...clientes.values()].some(c => c.username.toLowerCase() === alvo);
}
...
wss.on("connection", (socket) => {
  ...
  socket.on("message", (dados) => {
    ...
    if (msg.tipo === "entrar") {
      ...
      if (!username) {
        enviar(socket, { tipo: "erro", texto: "Nickname inválido." });
        return;
      }

      if (clientes.has(socket)) {
        return; // Já entrou nesta conexão
      }

      if (nicknameEmUso(username)) {
        enviar(socket, { tipo: "erro", texto: "Esse nickname já está em uso." });
        return;
      }
